...
use super::{provision_email_account, KratosWebhookPayload};
...
pub async fn enqueue_retry(
    ...
    if attempt >= MAX_RETRY_ATTEMPTS {
        warn!(
            "Maximum retry attempts ({}) reached for Kratos identity: {}. Not enqueuing.",
            MAX_RETRY_ATTEMPTS, payload.identity.id
        );
        return Ok(());
    }

    let retry_job = RetryJob {
        kratos_identity_id: payload.identity.id,
        payload,
        attempt: attempt + 1,
...
async fn process_retry_jobs(
    ...
    for job in jobs {
        info!(
            ...
            job.kratos_identity_id, job.attempt, MAX_RETRY_ATTEMPTS
        );
        ...
        match provision_email_account(db_pool, job.payload.clone()).await {
            Ok(response) => {
                info!(
                    ...
                    job.kratos_identity_id, response
                );
            }
            Err(e) => {
                error!(
                    "Retry attempt {} failed for {}: {}",
                    job.attempt, job.kratos_identity_id, e
                );
...
mod tests {
    ...
    #[test]
    fn test_retry_job_serialization() {
