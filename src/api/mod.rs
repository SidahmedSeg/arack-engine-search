...
use crate::{
    analytics::{AnalyticsManager, ClickEvent, SearchEvent}, // Phase 7.6-7.7
    auth::{
...
// Phase 8: Type aliases for authentication
...
async fn crawl(
    ...
    let job = crate::redis::CrawlJob::new(
        payload.urls.clone(),
        payload.max_depth,
...
async fn search(
    ...
    // Generate cache key
    let cache_key = crate::redis::CacheManager::search_cache_key(&params.q, params.limit, params.offset);
...
fn merge_search_results(
    keyword: Vec<crate::search::SearchResult>,
    semantic: Vec<crate::qdrant::ScoredPage>,
) -> Vec<HybridResult> {
