<script lang="ts">
	...
	// OAuth/SSO functions (Phase 9: Using SSO via account-service)
	async function loadOAuthStatus() {
		// Phase 9: User is connected via SSO session
		// If they can see this page, they're authenticated
		oauthStatus = { connected: true };
	}

	function connectOAuth() {
		// Phase 9: SSO is handled via arack.io/auth/login
		window.location.href = 'https://arack.io/auth/login?return_url=' + encodeURIComponent(window.location.href);
	}

	async function disconnectOAuth() {
		// Phase 9: Disconnect = logout from SSO session
		if (!confirm('Are you sure you want to sign out? You will be logged out from all Arack services.')) {
			return;
		}
		...
		try {
			...
			window.location.href = 'https://account.arack.io/api/logout?return_url=' + encodeURIComponent('https://arack.io');
		} catch (err: any) {
